...
/*   Updated: 2020/05/04 22:14:48 by macbook          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	++format;
	while (ft_is_format(*format) && (ft_is_flag(*format) ||\
	ft_is_conver(*format) || (*format >= '0' && *format <= '9')))
	{
		if (ft_is_flag(*format))
			format = ft_t_of_flag(*format, format, lst, t_save);
